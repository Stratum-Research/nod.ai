...
class OpenSourceProvider(BaseProvider):
    ...
    def check_model_downloaded(self, repo_id: str, filename: str) -> bool:
        ...
        try:
            ...
            print(f"Warning: Could not check download status for {repo_id}/{filename}: {e}")
            return False
    
    def _load_model(self, repo_id: str, filename: str, force_download: bool = False) -> Llama:
        ...
        llm = Llama.from_pretrained(
            ...
            filename=filename,
            verbose=False,
            download_dir=None if not force_download else str(Path.home() / ".cache" / "llama-cpp-python")
        )
    ...
    async def download_model(self, repo_id: str, filename: str) -> Dict[str, str]:
        ...
        def download_sync():
            ...
            try:
                ...
                return {"status": "downloaded", "repo_id": repo_id, "filename": filename}
            except Exception as e:
                ...
                try:
                    ...
                    if cached_path and Path(cached_path).exists():
                        ...
                        try:
                            ...
                            for related_file in cache_dir.glob(f"{Path(cached_path).stem}*"):
                        ...
                        return {"status": "deleted", "repo_id": repo_id, "filename": filename}
                    else:
                        return {"status": "not_found", "repo_id": repo_id, "filename": filename}
                except Exception as e:
        ...
        for model in self.registry:
            ...
            if not name:
                ...
                if len(parts) == 2:
                    ...
                    name = model_part.replace("-Instruct-GGUF", " Instruct").replace("-GGUF", "").replace("-", " ")
    ...
    def is_model_available(self, model_id: str) -> bool:
        ...
        if model_id.startswith("opensource:"):
            return self._get_model_info(model_id) is not None
        # Also check without prefix for convenience
        return self._get_model_info(f"opensource:{model_id}") is not None
